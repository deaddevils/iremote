...
function setSystem(stm) {
    console.log('android parameters：'+ stm);
    system = stm;
}
...
function onSubmit()
{
	...
	$.ajax({
        type: "post",
        url:"/iremote/device/editinfrareddevice",
...
<body class="listpage">
	<table class="title_table">
		<tr>
			...
			<td align="center"><span class="title_c"><s:property value="infrareddevice.name"/></span></td>
...
<form action="/iremote/device/seditinfrareddevice" id="formSumbit" method="post">
	<input type="hidden" id="infrareddeviceid" name="infrareddeviceid" value="<s:property value='infrareddeviceid'/>"/>
		...
		<table width="100%" style="background:#ffffff;margin-top:5px;">	
      	<tr height="55px">
			...
			<td align="left" >  
				<div>
					<input type="text" class="input_text" id="name" name="name" maxlength="32" value="<s:property value="infrareddevice.name"/>"/>
				</div>
			</td> 
		</tr>
		</table>
