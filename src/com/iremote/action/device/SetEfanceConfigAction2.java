...
public class SetEfanceConfigAction2 extends DeviceOperationBaseAction{
	...
	protected CommandTlv[] createCommandTlv() {
		if(calibrate==1){//jiaozhun
			return new CommandTlv[]{CommandUtil.createCalibrateCommand(device.getNuid(),(byte)channel)};
		}else{
			if(lowvoltage!=0){
				return new CommandTlv[]{CommandUtil.createLowVoltageCommand(device.getNuid(),(byte)channel,(byte)(lowvoltage/256),(byte)(lowvoltage%256))};
			}else{
				return new CommandTlv[] { CommandUtil.createBatchConfigCommand(device.getNuid(),(byte)channel, voltagechoose,
